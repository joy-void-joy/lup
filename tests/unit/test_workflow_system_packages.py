...
def test_a_declared_package_reaches_the_generated_gate() -> None:
    rendered = WorkflowSpec(system_packages=["poppler-utils"]).document().text()
...
def test_several_packages_share_one_install() -> None:
    ...
    rendered = (
        WorkflowSpec(system_packages=["poppler-utils", "pandoc"]).document().text()
    )
...
def test_declaring_none_renders_no_step() -> None:
    ...
    rendered = WorkflowSpec().document().text()
