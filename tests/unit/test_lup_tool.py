...
async def test_sdk_handler_serializes_output_as_json() -> None:
    resp = cast(ToolResponse, await echo.sdk_tool.handler({"text": "hi"}))
...
async def test_tool_error_becomes_is_error_response() -> None:
    resp = cast(ToolResponse, await echo.sdk_tool.handler({"text": "boom"}))
...
async def test_invalid_input_becomes_is_error_with_message() -> None:
    resp = cast(ToolResponse, await echo.sdk_tool.handler({"wrong_field": 1}))
    assert resp.get("is_error") is True
...
def test_models_inferred_from_annotations() -> None:
    ...
    assert echo.sdk_tool.name == "echo"
    assert echo.sdk_tool.input_schema == EchoInput.model_json_schema()
