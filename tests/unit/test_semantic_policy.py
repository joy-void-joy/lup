...
SHELL_POLICY_CASES = [
    ...
    DecisionCase(input="awk '{print > \"out\"}' f", effect="ask"),
    DecisionCase(input="awk 'BEGIN{system(\"id\")}'", effect="ask"),
    DecisionCase(input="seq 3 | awk '{print | \"sort\"}'", effect="ask"),
    DecisionCase(input="gawk -i inplace '{gsub(/a/,\"b\")}1' f", effect="ask"),
    DecisionCase(input="awk -f prog.awk f", effect="ask"),
