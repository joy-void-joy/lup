...
async def test_notification_iterator_stops_at_the_terminal_sentinel() -> None:
    queue: asyncio.Queue[RpcNotification | None] = asyncio.Queue()
    queue.put_nowait(RpcNotification(method="turn/started"))
    queue.put_nowait(RpcNotification(method="turn/completed"))
    queue.put_nowait(None)

    methods = [item.method async for item in notification_iterator(queue)]
