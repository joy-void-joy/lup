...
| `.claude/plugins/lup/hooks/`, `.codex/plugins/lup/hooks/` (`hooks.json`, `scripts/policy.py`, `runtime/policy_data.py`, `runtime/evidence.json`) | `catalog.py` `HookSet` via the adapter hook renderers and `lup.policy.bundle` |
...
| `.claude/workflows/commands/resolve.js` | `lup.adapters.claude.harness` |
