...
| Pattern catalogs | `docs/orchestration.md`, `docs/patterns.md` | same two files | Parity — single copy at a neutral path. Both render identically under either prompt renderer (one text part, no invocation parts, no native spellings), so a per-tree copy would be a byte duplicate, and a full path resolves the same wherever the reader's guidance file sits — `AGENTS.md` is pinned to the repository root by Codex's own discovery convention, so a bare filename would have been ambiguous while a full path is not. They join `docs/self-improvement.md` and `docs/permissions.md` at neutral locations; a second rendered copy becomes worthwhile only if either document gains invocation parts. |
