...
| OS sandbox boundary (uncontained session) | The `HookSandbox` declaration compiles into the `settings.json` `sandbox` block (bwrap network allowlist, human-owned write denials, credential read denials); the launcher exercises `bwrap --version` and `socat -V` — each flag declared beside its own program in `lup.harness.toolchain`, because one flag spelled for both reported socat broken everywhere — before exporting `LUP_SANDBOX_ACTIVE` | The launcher establishes an explicit `--sandbox workspace-write` envelope on the interactive command line and exports the flag only for an envelope it set itself; a caller-supplied sandbox flag keeps the deny lattice active | Codex sandbox config has no per-path write denials or domain allowlist, so its envelope is the declaration's strict subset (network off); the resolver paths carry their own explicit envelopes on both platforms. |
| OS sandbox boundary (contained session) | `--settings {"sandbox": {"enabled": false}}` merged at launch; the artifact still declares `enabled: true`, which is the right answer for the uncontained launch the same file serves | `--sandbox danger-full-access`, which is what the Codex documentation prescribes once a container provides the isolation | The container is the boundary and the kernel reads it from the image's own `LUP_CONTAINED`, so `LUP_SANDBOX_ACTIVE` is neither needed nor exported and no dependency is probed. Measured on Claude: bubblewrap cannot mount a fresh `/proc` in an unprivileged container, so the inner sandbox never started and the packages installed for it bought silence rather than a boundary. On Codex the stakes are the reverse — `workspace-write` turns network access off, which would cut a contained session off from the egress proxy that is its only route out. |
...
| --- | --- | --- | --- |
| Skills (32) | `commands/*.md` | `skills/*/SKILL.md` | Parity — same 32 declarations, native formats. |
