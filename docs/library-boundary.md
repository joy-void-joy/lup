...
any one module is judged. `TREE_VALIDATORS` is not flagged: `validated_tree`
takes it through the sentinel spelling.
...
Twenty-seven tables are in scope. Twenty-two are canonical and carry their
reason at the site. Five are application data with no parameter — these are the
...
explain *why* those five and not the other twenty-two. The pressure to act lives
