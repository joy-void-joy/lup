...
| `.claude/plugins/lup/hooks/runtime/kernel/`, `.codex/plugins/lup/hooks/runtime/kernel/` (13 modules) | verbatim copy of `lup/policy/kernel/`, read by `policy_kernel_modules()` and kept byte-identical so it can be diffed against the canonical package |
