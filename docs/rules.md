...
| `set-shape` | anti-pattern | Python | <code>(?&lt;!\.)\bset[\[(]&#124;(?::&#124;-&gt;)\s*set\b</code> | A declared `set` is usually better as a dict, when the members key something, or as a `list[BaseModel]`, when each member carries more than its own name — a bare set of strings is often a record that lost its other fields. Reach for membership on a local set comprehension instead of declaring the set as the interface. For a genuinely set-shaped value add `# lup: ignore[set-shape]` | `lup.codescan.antipatterns` |
