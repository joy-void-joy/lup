...
class ClaudeProfileSupport(ProfileSupport):
    ...
    def select(self, name: str | None, client: Client) -> Client:
        ...
        returned client is a copy whose native options carry the
        resolved dir; the given client is left untouched.
        """
        from lup.adapters.clients.claude.client import ClaudeClient

        match client:
            case ClaudeClient():
                native = copy.copy(client.options)
                native.env = {
                    **native.env,
                    CONFIG_DIR_ENV: str(self.resolve_config_dir(name)),
                }
                selected = copy.copy(client)
                selected.options = native
                return selected
            case _:
                raise TypeError(
                    "ClaudeProfileSupport selects accounts on Claude clients; "
