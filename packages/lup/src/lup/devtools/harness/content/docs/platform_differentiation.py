...
| Escaping the sandbox for one call | a per-call flag on the launching tool | declined, with the reason | `NativeSpellings.escape_sandbox` returns a `Spelling`, so a runtime whose only overrides are session-level flags on its binary answers `Unsupported` instead of naming a flag its reader cannot pass. Both resolver entries ask it, so neither can hardcode an escape and neither can be silently absent. |
