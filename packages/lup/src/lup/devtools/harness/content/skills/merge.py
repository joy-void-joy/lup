...
- **`"""
            ...
            models.TextPart(
                text=r""" <target>`** — Merge `<target>` branch into the current branch, with intelligent conflict handling.
- **`"""
            ),
            models.SkillInvocation(plugin="lup", skill="merge"),
            models.TextPart(
                text=r"""`** (no argument) — Detect and resolve conflicts from an in-progress merge, rebase, or cherry-pick.
...
Invoke `"""
            ...
            models.TextPart(
                text=r"""` to commit any uncommitted work before starting the merge. This ensures a clean working tree and a restore point if the merge goes wrong.
...
Which spelling to use depends on whether the target already carries this branch. Where it does -- `sync-base` pulled the integration branch in before the rebase, which is the route `"""
            ),
            models.SkillInvocation(plugin="lup", skill="land"),
            models.TextPart(
                text=r"""` arrives by -- the join already happened in the branch's own worktree, and recording it again here would put a merge commit in an integration branch whose history is otherwise linear:
