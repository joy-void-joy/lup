...
The topic roster and the path are read from the report surface's own
declaration rather than restated here, so the skill cannot come to describe a
report the command does not produce. Only the models are imported — nothing
that wires a CLI — because this prose is generated by that CLI.
"""

import lup.harness.models as models
from lup.devtools.report.models import DEFAULT_REPORT_PATH, topic_bullets
...
Write down everything there still is to implement, at `{DEFAULT_REPORT_PATH}`.
...
The target is `{DEFAULT_REPORT_PATH}` unless the invocation names a file — an
...
Read `{DEFAULT_REPORT_PATH}`, or the stdout you just captured. Those sections
...
- **One report file, and only one.** `{DEFAULT_REPORT_PATH}`, or the file the
