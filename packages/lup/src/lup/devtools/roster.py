...
import lup.devtools.ledger.app as ledger_tree
...
class DevtoolsDeclarations(BaseModel, frozen=True, arbitrary_types_allowed=True):
    ...
    def roster(self, retired: list[str] | None = None) -> list[SubApp]:
        ...
        *not built* rather than built and dropped afterwards. That distinction
        ...
        A writeup is regenerated by the ledger's own tree, and its banner and
        stamp say so. A roster without that tree writes none: the file would
        name a command nothing serves, and the documented-commands sweep would
        refuse what generation had just written.
        """
        declined = retired or []
        wired = (
            self.model_copy(update={"writeups": []})
            if ledger_tree.SUBAPP_SPEC.name in declined
            else self
        )
        return [
            entry.wired(wired)
...
LIBRARY_ROSTER = [
    ...
    RosterEntry(
        spec=ledger_tree.SUBAPP_SPEC,
        build=lambda declared: ledger_tree.create_ledger_app(
