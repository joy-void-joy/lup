...
reading and :func:`default_subapps` the other. Both project the same entries,
so a roster the documents describe and a roster the CLI serves cannot disagree.
"""
...
def default_subapps(declared: DevtoolsDeclarations) -> list[SubApp]:
    """The same roster, wired over what one project declares about itself."""
    return [entry.wired(declared) for entry in LIBRARY_ROSTER]
