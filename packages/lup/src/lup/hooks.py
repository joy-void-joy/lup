...
def create_permission_hooks(
    ...
    async def permission_hook(
    ...
    ) -> LupHookOutput:  # lup: Sounds LupHookInput is very low level. But hooks.py is higher level, and should interface with a high-level normalized BaseModel
        ...
        match tool_name:
            case (
                "Write" | "Edit"
            ):  # lup: Yeah, no this is very untyped here. Doesnt' generalize to Codex, etc.
    ...
    return {
        "PreToolUse": [
            LupHookMatcher(hook=permission_hook, tag="permission")
        ],  # lup: Same comment here
    }
...
def create_tool_allowlist_hook(
    ...
    **Why:** Makes tool availability a structural guarantee instead of a
    ...
    async def allowlist_hook(input_data: LupHookInput) -> LupHookOutput:
        ...
        ):  # lup: Same, this file really needs thorough refactoring
            return LupHookOutput()
