...
| Waking an idle peer | `wake()` writes one newline-delimited JSON frame (`{{"type":"user","message":{{...}}}}`) to the session's own inbox socket, whose path the member declares; the session takes a turn on it | `codex queue --thread <uuid or name> --message <text>`, which reaches a session from any process on the machine | Both runtimes are reached from any process, by means that are each the runtime's own, so the library finishes the job on both and the asymmetry earlier recorded here does not exist. A wake is always *on top of* the mail, never instead of it, so the record is identical and only the latency differs. Measured on Claude Code 2.1.278 rather than inferred: a background session confirmed idle by `claude agents --json` (`status: idle`) was sent a frame by a plain script and answered a token it could only have read there; the same again through an inbox relocated by `--messaging-socket-path`. The authentication frame the runtime's own startup text calls required is **not** needed — a frame sent without one was accepted — so reachability of the socket is the whole of the credential, which is why the directory those sockets live in is the security boundary and why lup places them rather than sharing the runtime's default directory. Claude's CLI has eighteen verbs at this version and none speaks to a live session; the path is the socket, not a verb. Discovery is separately two paths — a scan of the socket directory, which containment makes container-local, and a registry under `$CLAUDE_CONFIG_DIR/sessions/` carrying `messagingSocketPath`, `name` and `status`, which **only `kind:"bg"` sessions publish to** — so an interactive peer is invisible to a native listing while still being woken by path. A member that declared no path is told so rather than silently un-nudged. What `codex queue` does to an *idle* session — wakes it, or queues for its next turn — is still unmeasured: no signed-in Codex session was available, so `wake()` reports `reached` on a zero exit, which is the honest reading of the command having succeeded and not of the peer having looked. The Windows named-pipe transport, and the uncontained and docker postures, are likewise unmeasured. |
