...
| Device lease | Declared on the `Image` (`devices`) or asked for with `--device`, resolved against the host's CDI registry, and rendered as `--device` on the container both runtimes launch through | Same, from the same declaration | A device is a property of the container, and one `Image` declaration starts every runtime. The host postures (`--sandbox inner`, `--sandbox none`) run on the host, which holds its own devices, so neither runtime spells anything there; a `--device` under one is answered with a notice saying so. |
"""
            ),
            *resolver_entry,
            models.TextPart(
