...
- **Tool errors**: For each failure — what happened (quote the error), why it failed (read the tool source in {{ agent_tools_directory }}), was recovery reasonable.
- **Subtle bugs**: Cases where a tool *succeeded* but returned misleading or incomplete data.
- **Missing tool calls**: Tools the agent *should* have called but didn't. Check available tools in {{ agent_tools_directory }}.
