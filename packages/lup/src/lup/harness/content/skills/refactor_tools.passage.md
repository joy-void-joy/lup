...
1. Read {{ agent_toolsets_py }} first. It is the tool-group
...
2. Read {{ agent_core_py }} to understand:
...
3. Read {{ agent_tool_policy_py }} to understand:
   - Which tools are gated by conditions (API keys, modes, etc.)
   - Are there tools registered but conditionally unavailable?

4. Read {{ agent_subagents_py }} and
   {{ agent_tools_nested_py }}. Delegation comes in four
...
Read every file in {{ agent_tools_directory }} and build a **tool outline** — not just a table, but a readable summary of each tool's purpose.
...
- Read {{ agent_core_py }} for `LupMcpServerConfig` values and
  the server list they are assembled into
- Read {{ agent_config_py }} for MCP-related settings
