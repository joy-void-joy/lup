...
This is distinct from `{{ add_command_skill }}` which creates general-purpose commands. Investigator commands share a specific pattern: raw input in, traced diagnosis out.
...
1. **Read existing investigator declarations** as reference. `debug` is the worked example — it traces errors through logs, and it is the library's, so read it through the package rather than through a path that only resolves when lup is vendored:
   ...
   ```bash
   uv run lup-devtools dev py source lup.harness.content.skills.debug
   ```
...
Show the user what was created — the first draft is rarely perfect. {{ ask_2 }}
