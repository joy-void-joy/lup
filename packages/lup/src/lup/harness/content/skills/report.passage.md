...
Write down everything there still is to implement, in a file under `{{ default_scratch_root }}/`
...
{{ default_scratch_root }}/plan_something.md", the same report is written again from scratch, saying
...
`{{ default_scratch_root }}/<the work it covers>.md`. What is left of `{{ default_scratch_root }}/plan_relocated_notes.md`
is `{{ default_scratch_root }}/relocated_notes_remaining.md`; a session spent on the report surface
leaves `{{ default_scratch_root }}/report_surface_remaining.md`. The name is what the next session
...
```bash
uv run lup-devtools dev report --write {{ default_scratch_root }}/<name>.md --force
```
...
The path has to sit under `{{ default_scratch_root }}/` or the command refuses it: that is where a
...
- **One report per piece of work, named for it.** Under `{{ default_scratch_root }}/`, which is
