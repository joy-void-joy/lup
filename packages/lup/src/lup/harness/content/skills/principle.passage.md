...
Lesson table in the guidance), and 2-3 concrete examples of it in action. Then {{ ask }}
...
1. **Guidance** ({{ harness_content_guidance_py }}, plus
   ...
   - {{ guidance_file_path }} are generated from this module — read them for the rendered result, never edit them

2. **Template guidance** ({{ harness_content_template_sections_py }})
   - Same checks — this is what new projects inherit; the portable sections render into every flavor of {{ guidance_template_path }} from this one source

### Layer B: Commands & Workflows

3. **All skill modules** (`{{ harness_content_skills_directory }}*.py`,
   ...
   not vendored here, reach its modules with `uv run lup-devtools dev py source`
   ...
   - {{ skills_path }} are generated from these — never edit them

### Layer C: Semantic Policy & Enforcement

4. **Canonical policy** (the library's `lup.policy` plus the `HookSet` in
   {{ harness_catalog_py }}; everything under {{ hooks_path }} is generated from these — never edit it directly)
...
7. **Environment code** ({{ environment_directory }})
...
**Group 5: Devtools & automation** (`lup.devtools` and {{ devtools_directory }})
