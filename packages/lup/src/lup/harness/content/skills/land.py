...
- **On a feature branch, where the work is that branch's own.** Invoke `"""
            ),
...
- **Open a PR** — get into that branch's worktree: """
            ...
            models.TextPart(
                text=r""". Create one first via `uv run lup-devtools git worktree create <branch>` when `worktree` is null. Then run `"""
            ),
