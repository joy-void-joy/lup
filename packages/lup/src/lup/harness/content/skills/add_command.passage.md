...
- `{{ add_command_skill }} review` — name is `review`, description not provided (ask)
- `{{ add_command_skill }} review Analyze PR diffs and suggest improvements` — name is `review`, description is "Analyze PR diffs and suggest improvements"
...
1. Decide which half owns it: a skill that automates work *inside* a project is the library's (`lup.harness.content.skills`); a skill whose subject is standing a project up or keeping it in step with upstream is this project's ({{ harness_content_skills_directory }}). Create `<name>.py` there, exporting a `SKILL`:
...
{{ skills_path }} are written by that step — never by hand.
...
3. Explain how to invoke it: `{{ skill_pattern }}`
