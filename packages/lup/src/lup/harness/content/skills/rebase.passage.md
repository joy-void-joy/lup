...
Run `uv run lup-devtools git pr sync-base --json` (step 1 below) -- it reports the base branch and a `base_source`. `recorded` (from worktree creation) and `explicit` are authoritative. `guessed` means topology alone picked it: the command merges nothing and exits non-zero. {{ ask }}, then rerun as `sync-base --base <branch>`.
...
Invoke `{{ commit_skill }}` to commit any uncommitted work before starting the rebase.
...
A `guessed` base exits non-zero having merged nothing -- settle the base as above and rerun with `--base <branch>`. If conflicts are reported, resolve with `{{ merge_skill }}` (no argument) first.
...
When it is ahead, this is the user's call, not yours, because one of the answers publishes their unpushed work. {{ ask_2 }}. Rebuilding means cherry-picking this branch's own commits onto `origin/<base>`, which touches nothing of theirs; verify it as step 8 does before going on.
...
Check whether `{{ personal_settings_path }}` exists. Its permission entries are ad-hoc grants one person accepted; leaving them there means the next person re-approves the same prompts.

`{{ project_settings_path }}` is a generated artifact (`harness.project-settings`) -- never hand-merge into it. The next `lup-devtools harness generate all` regenerates it from the policy and drops the edit. Shell, fetch, and edit permissions belong to the canonical semantic policy instead.

Classify each entry:

- **Already covered by the policy** -- drop it from the local file. Most read-only commands are.
- **A genuine gap** -- invoke `{{ hooks_skill }}` to add it to the canonical policy, regenerate both native plugins, and commit that as a separate commit.
- **User-specific** (plugin toggles, personal model choice) -- leave it in `{{ personal_settings_path }}`.
