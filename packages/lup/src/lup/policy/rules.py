...
class ShellPolicy(DecisionPolicy[ShellCommand]):
    ...
    def __init__(
        ...
        # Two facts about the session rather than about any command, and
        # neither is discoverable from here: whether a container is the
        # boundary, and whether the undo layer holds this tree. A caller that
        # knows says so, and one that does not gets the strict answer -- which
        # for both is the one that relaxes nothing.
