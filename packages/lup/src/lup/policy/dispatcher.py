...
DISPATCHER_STDLIB = ("json", "os", "sys", "pathlib")
...
def compiled_docstring(declaration: DispatcherDeclaration) -> str:
    """Name the compiled script's runtime and both sources it was built from."""
    return (
        f'"""Generated {declaration.runtime_name} hook dispatcher over the '
        "canonical semantic kernel.\n"
        "\n"
        "Compiled by lup.policy.dispatcher from "
        f"{SHARED_PACKAGE}.assets.{SHARED_MEMBER} and\n"
        f"{declaration.package}.assets.{RUNTIME_MEMBER} — do not edit directly.\n"
