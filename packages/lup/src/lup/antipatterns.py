...
auditor show. This module imports only the standard library and `pydantic` so
the hook and the auditor can load it cheaply; `# lup:` marker detection itself
stays in `lup.markers`, which both this set's consumers and the auditor import
directly.
"""
...
def audit_text(text: str, patterns: list[AntiPattern]) -> list[AntiPatternFinding]:
    ...
    def inline_directive(line_no: int, line: str) -> re.Match[str] | None:
        ...
        if match is None:
            return None
        if (
            comment_columns is not None
            and comment_columns.get(line_no) != match.start()
        ):
            return None
        return match
