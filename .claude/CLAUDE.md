...
2. `EnterWorktree(path=<the path step 1 prints>)`, returning afterwards with `ExitWorktree(action="keep")` — creating a worktree does not move the session, and edits left in the old checkout never reach the branch.
