...
Examples::
    ...
    $ uv run lup-devtools pr status --json
    $ uv run lup-devtools pr merge 42
    $ uv run lup-devtools pr sync-base --json
    $ uv run lup-devtools pr push --force --json
    $ uv run lup-devtools pr create --base dev --title "feat: search" --body "..."
    $ uv run lup-devtools pr update 42 --body "..."
...
from lup.devtools.branch import detect_integration_branch
...
def find_base_branch() -> str:
    ...
    branches = str(git("branch", "--format=%(refname:short)")).strip().splitlines()
    candidates = [b for b in branches if b != branch]

    if not candidates:
        return detect_integration_branch()

    best_branch = detect_integration_branch()
    best_distance = float("inf")

    for candidate in candidates:
        try:
            merge_base = str(
                git("merge-base", head, candidate, _ok_code=[0, 1])
            ).strip()
            if not merge_base:
                continue
            count_str = str(
...
def status_cmd(
    ...
    detail_raw = str(
        gh("pr", "view", str(pr_number),
           "--json", "reviews,statusCheckRollup,mergeable,mergeStateStatus,reviewDecision")
    ).strip()
