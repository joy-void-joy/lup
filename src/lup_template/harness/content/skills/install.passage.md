...
- {{ project_settings }} — each tree's project configuration
...
- **Plugin infrastructure**: The plugin directory itself — {{ root_plugin_path }}
...
- **DevTools**: The full `lup-devtools` CLI ({{ subapp_summary }})
...
trees the target carries: {{ ask_user }} In non-interactive mode, install the trees the target already shows evidence
...
Create the plugin directory in each selected tree ({{ root_plugin_path_2 }}) as a standalone local plugin in the target repo. Per tree, this requires:

1. **Plugin manifest** ({{ manifest_plugin_path }}) — the plugin entry's `name` stays `lup`, so every skill keeps its `lup:` qualifier.
2. **Marketplace registration** ({{ marketplace }}) with a **project-unique** `name` (use the target's package/repo name, e.g. `myproject`), listing the `lup` plugin:
   ```json
   { "name": "myproject", "plugins": [{ "name": "lup", "source": "./lup" }] }
   ```
3. **Register + enable** it in that tree's project configuration ({{ project_settings_2 }}) under the same unique name. Each runtime spells this differently — read
...
| marketplace `name` (marketplace.json) → `<target>` | plugin entry `name`: `lup` (so `{{ skill_pattern_2 }}` is stable) |
...
1. Plugin manifest — {{ manifest_plugin_path_2 }}
2. Hook definitions and adapted hook scripts — {{ hooks_plugin_path }} (only reference hooks being installed)
3. Marketplace registration — {{ marketplace_2 }}
4. Selected skills — {{ skills_plugin_path }}
...
6. Project configuration — {{ project_settings_3 }} — create or merge
7. Guidance file — {{ guidance_file_2 }} — section-level merge from that tree's template flavor (read template → use `<!-- section: ... -->` markers to identify merge units → adapt for target → compare sections → add missing ones → leave existing untouched)
