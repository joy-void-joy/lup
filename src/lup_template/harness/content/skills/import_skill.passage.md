...
local repository, sibling worktree, or branch into this project. Unlike `{{ update_skill }}` which reviews a configured downstream as a whole, this
...
- `{{ import_skill }} forecaster version reviewer pattern` — tracked project
`forecaster`, with the exact base and source revisions resolved before review
- `{{ import_skill }} ../feat-boundary dev..HEAD all non-sandbox features` —
the sibling worktree at `../feat-boundary`, explicit range `dev..HEAD`, and a
scope that excludes sandbox-only slices
- `{{ import_skill }} feature/ref clipboard workflow` — a ref in this
...
other direction and another skill: `{{ update_skill }}` moves every carrier at once and reads no commits.

If only a source selector is provided (single word, no description), {{ ask }}

## Steps

### 1. Commit pending changes

Invoke `{{ commit_skill }}` to commit any uncommitted work before importing.
...
Present your analysis and proposed adaptation plan, then {{ approval }}
...
Offer to commit the imported pattern, through `{{ commit_skill }}` as in step 1 — it groups the change and writes the message in a form the permission policy allows, which a hand-rolled `git commit` here would have to get right again.
