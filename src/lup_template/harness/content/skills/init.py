...
"""
            ...
            models.AskUser(
                question="which modules this domain takes and which it declines, "
                "one option per module the roster offers"
            ),
            models.TextPart(
                text=r""" — for each, say which way you would go and why, from what the interview
...
The merge lands in `src/<project>/harness/content/guidance.py`, never in a tree's guidance file ("""
            ),
...
Edit `src/<project>/harness/content/guidance.py`, then regenerate with `uv run lup-devtools harness generate all` -- """
            ),
            models.NativePath(location="guidance_file", scope="every_tree"),
            models.TextPart(
