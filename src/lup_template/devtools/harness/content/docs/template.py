...
# The template guide names the trees `harness` generates and launches.
"""Guide to ``src/lup_template``, the application built on the library."""

import lup.harness.models as models

from lup_template.devtools.subapps import subapp_bullets
...
"""
        ...
        models.TextPart(text=subapp_bullets()),
        models.TextPart(
            text=r"""
...
`/lup:update` and `/lup:import` skills are built on it. Two files declare
what to track.
...
`/lup:update` can generalize emerged patterns back into the template. Same
