...
- **A `# lup: defer: <text>` note**, when the work belongs to a site in this code, where `dev check` keeps it visible until somebody wakes it. Default to the bare `defer:`; a bracketed `defer[<gate>]: <text>` states a real, externally-checkable gate, never that this code might change again. Two spellings are resolved rather than read — `defer[branch:<name>]` wakes once that branch reaches the integration branch, `defer[gone:<path>]` once that path stops existing — and `dev check` fails the run either comes true. Reach for one where it fits: any other gate stays prose, which means it stays advisory, and a dormant note is one nobody has reason to read until the moment it stops being dormant.
