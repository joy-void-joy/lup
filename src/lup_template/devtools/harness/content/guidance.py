...
Work in a **git worktree**, not a branch switched in place, and never commit _code_ directly to `dev`. Creating one does not move the session, and edits left in the old checkout never reach the branch — so create it with `uv run lup-devtools dev worktree create feat-name`, which lands it as a sibling under `tree/` and never nested inside another checkout, and then """
        ),
