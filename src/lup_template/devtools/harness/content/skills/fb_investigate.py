...
```
Agent(subagent_type="lup:trace-explorer", prompt="Investigate session <session_id> following the per-session steps in """
            ),
            models.SkillInvocation(plugin="lup", skill="fb-investigate"),
            models.TextPart(
                text=r""". Report: tool call inventory, errors with quoted output, workflow assessment, outcome classification, counterfactuals.")
```
