...
- `{layout.path("agent", "core.py")}` — composition: how a factory is built and wrapped
- `{layout.path("agent", "toolsets.py")}` — the tool-group registry, the one place a group is added
- `{layout.path("agent", "tools", "example.py")}` — the worked tool pattern
- `{layout.path("agent", "tools", "nested.py")}` — the copyable nested-agent template
- `{layout.path("agent", "tools", "realtime.py")}` — persistent-agent tools (sleep, context, reply)
- `{layout.path("agent", "tools", "reflect.py")}` — the reflection tool and its reviewer
- `{layout.path("agent", "models.py")}` — the output model
...
When you need to verify a capability of the runtime itself, read """
                ),
                models.RuntimeDocs(),
                models.TextPart(
                    text=r""" rather than answering from memory. Fetch or search it for specifics, and delegate to whatever documentation agent your harness ships when it has one.
...
runtimes to drive, which delegation shape a job wants — """
                ),
                models.AskUser(
                    question="which of the approaches just described to design around"
                ),
                models.TextPart(
                    text=r""" rather than letting it pass by in prose. For open-ended exploration, ordinary conversation is right.
