...
local repository, sibling worktree, or branch into this project. Unlike `"""
            ),
            models.SkillInvocation(plugin="lup", skill="update"),
            models.TextPart(
                text=r"""` which reviews a configured downstream as a whole, this
...
- `"""
            ...
            models.TextPart(
                text=r""" forecaster version reviewer pattern` — tracked project
`forecaster`, with the exact base and source revisions resolved before review
- `"""
            ),
            models.SkillInvocation(plugin="lup", skill="import"),
            models.TextPart(
                text=r""" ../feat-boundary dev..HEAD all non-sandbox features` —
...
If only a source selector is provided (single word, no description), """
            ...
            models.AskUser(question="what feature or scope to import from that source"),
            models.TextPart(
                text=r"""
