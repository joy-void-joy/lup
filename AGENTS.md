...
Change the policy those gates enforce with $lup:hooks, which edits the canonical inputs in `lup.policy` and the `HookSet` in `devtools/harness/catalog.py`, regenerates both native plugins, and runs the shared fixture suite. Harness generation compiles one hermetic dispatcher and runtime per plugin; never edit a generated dispatcher or runtime, and keep harness settings project-level, in the tree the harness owns (.codex/config.toml), which holds only the native settings outside that semantic policy boundary — never user-level.
