...
- **Open a PR** — get into that branch's worktree: start a session rooted at <the survey's worktree field> and continue there — this runtime cannot move a running session, so work carried on here would land in the checkout it started from. Already running, address files there by absolute path, where that tree is writable, which reaches the same branch.. Create one first via `uv run lup-devtools git worktree create <branch>` when `worktree` is null. Then run `$lup:rebase`.
...
- **Merge it from here** — the route when that branch's worktree is held read-only, which is a resolver worker's lease: `worker_lease` in `lup.sandbox.rail` mounts every sibling worktree that existed when the worker started read-only, so `$lup:rebase` cannot run there at all: its first write is refused before it reaches a file, and neither a sandbox escalation nor a command exclusion lifts a mount. An operator's contained session is under the other lease, `lease_for`, which holds every checkout of the repository writable — the routes above are open to it, and what it meets instead is the two mounts below. Merge from the integration checkout — `git merge <branch>`, then `uv run lup-devtools harness generate all` until it reports `ownership=present`, committing what that writes, then `uv run lup-devtools dev check`. One branch at a time, so a failing check still names the branch that caused it. No history is rebuilt, so this suits commits that are already atomic; what it buys over the routes above is that the checks run on the integrated result rather than on each branch alone.

  **Where the integration checkout itself holds a read-only mount on a file the fast-forward rewrites, the merge is the user's, from a host terminal.** A contained session mounts each human-owned file — `README.md` here — read-only over its own checkout, and git replaces a file by unlinking it, which a mount point refuses: `unable to unlink old 'README.md': Device or resource busy`. Detect it before merging: `findmnt -l -o TARGET,OPTIONS | grep -E '\bro\b'` lists every read-only mount, and any of them under the checkout that `git diff --name-only <integration>...<branch>` also names is one the merge cannot write. A `# lup: escalate[sandbox]:` marker does not help — it lifts the runtime's per-call sandbox, never a mount, and the approved command runs in the same mount namespace, where the refusal stands. Hand the user the exact command, `git -C <checkout> merge --ff-only <branch>`, and wait for it. A merge tried inside first aborts mid-checkout and leaves that worktree dirty with HEAD unmoved: the files git wrote before the failure show as modified, the ones it created as untracked. Restore with `git restore --worktree -- .`, then `git clean -f -- <each untracked path it wrote>`; the policy refuses `git checkout -- .`, so spell `git restore`.

  **Where the branch's worktree existed when the session started, its directory is a mount point, and `git delete` cannot remove it from inside.** A contained session bind-mounts every checkout that existed at launch at its own path, writable, and a mount point cannot be removed from within its namespace: `git worktree remove`, the first step of `uv run lup-devtools git delete <branch>`, empties the checkout and unregisters it, then fails on the directory with `failed to delete '<path>': Device or resource busy`, so `git delete` stops there with the branch and origin's copy untouched. The message is git's own: the attribution names a read-only mount, and this one is writable. Nothing is lost where the work already landed — `git delete <branch>` run again finds no worktree and deletes the branch, and the empty directory is the host's to remove once the container is gone. Say so rather than reporting the directory gone. A worktree cut inside the session is no mount point and removes whole.
