...
- Every marker still present belongs to another concern, or is `# lup: defer:` parked work you were not asked to wake — the bare spelling is the default, and a bracketed `defer[<gate>]:` states a real, externally-checkable gate ("until the v2 API ships"), never a restatement that this code might change again — with `defer[branch:<name>]` and `defer[gone:<path>]` resolved by `dev check` rather than read. Leave them all in place. If resolving your concern means deleting or moving code that carries one, do so and name it in your summary.
