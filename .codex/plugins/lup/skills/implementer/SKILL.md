...
- Your concern's `# lup:` review markers are already gone from this worktree; the orchestrator removed them before you started, so the spec is the whole of the feedback. **Do not** re-introduce a marker, and **do not** leave an explanatory comment where one stood — reshape the code so it reads on its own.
- Every marker still present belongs to another concern, or is `# lup: defer[<wake condition>]:` parked work behind a condition you were not asked to meet. Leave them all in place. If resolving your concern means deleting or moving code that carries one, do so and name it in your summary.
